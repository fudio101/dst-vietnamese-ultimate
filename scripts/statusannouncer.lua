...
function StatusAnnouncer:AnnounceRecipe(recipe, ingredient)
    ...
    if ingredient == nil then
        ...
        -- if string.find(announce_str, "\?\.$") ~= nil then (old version)
        ...
        local has, num_found = ThePlayer.replica.inventory:Has(ingredient,
            RoundBiasedUp(amount_needed * ThePlayer.replica.builder:IngredientMod()))
...
function StatusAnnouncer:RegisterCommonStats(
    ...
    if boat ~= false and type(status.boatmeter) == "table" then
        self:RegisterStat(
            ...
            stat_categorynames,
            function(player)
                local boat = player.components.walkableplatformplayer and
                player.components.walkableplatformplayer.platform
                local healthsyncer = boat and boat.components.healthsyncer
                if not (healthsyncer and healthsyncer.max_health) then
                    return 0, 0
                else
                    return math.ceil(healthsyncer.max_health * healthsyncer:GetPercent()),
                        healthsyncer.max_health                                                     -- Klei yydsb
                end
            end,
            switch_fn
        )
    ...
    if pethealth ~= false and ThePlayer.components.pethealthbar ~= nil and type(status.pethealthbadge) == "table" then
        self:RegisterStat(
            ...
            function(ThePlayer)
                return
                    math.floor(ThePlayer.components.pethealthbar:GetMaxHealth() *
                    ThePlayer.components.pethealthbar:GetPercent() + 0.5),
...
function StatusAnnouncer:OnHUDControl(HUD, control)
    ...
    elseif HUD:IsControllerInventoryOpen()
        ...
        if control == CONTROL_MAP and has_seasons(HUD, true) then
            return self:AnnounceSeason()
        end
    end
end
